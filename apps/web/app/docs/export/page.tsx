      ...
      <article className="min-w-0 flex-1 max-w-3xl">
        <div className="mb-8 pb-6 border-b border-outline-variant">
          ...
          <p className="mt-3 text-base leading-relaxed text-on-surface-variant">
            The export pipeline runs frame-by-frame in a dedicated Web Worker, muxes MP4 with
            mediabunny, and never drifts from the live preview.
          </p>
        </div>
        ...
        <section className="mb-10">
          ...
          <p className="mb-4 text-sm leading-relaxed text-on-surface-variant">
            ...
            is the primary export entry point. It spins up the export worker, renders every frame to{' '}
            <code className="rounded bg-surface-container px-1.5 py-0.5 text-xs font-mono">
              OffscreenCanvas
            </code>
            , muxes the result, and resolves with an MP4{' '}
            <code className="rounded bg-surface-container px-1.5 py-0.5 text-xs font-mono">
              Blob
            </code>
            :
      ...
      {progress && (
        ...
        <section className="mb-10">
          ...
          <p className="mb-4 text-sm leading-relaxed text-on-surface-variant">
            ...
            ) as the live renderer — so preview and export never drift.
          </p>
          <div className="overflow-hidden rounded-md border border-outline-variant mb-4">
            {[
              {
                step: '1',
                label: 'Serialize project',
                desc: 'Main thread transfers the Project snapshot and source blobs to the worker',
              },
              {
                step: '2',
                label: 'Frame loop',
                desc: 'Worker calls resolveTimeline(frame, project) for each frame and draws to OffscreenCanvas',
              },
              {
                step: '3',
                label: 'VideoEncoder',
                desc: 'Each canvas frame is encoded via WebCodecs VideoEncoder with the configured codec/bitrate',
              },
              {
                step: '4',
                label: 'Audio mix',
                desc: 'Audio is decoded and mixed on the main thread (Web Audio not available in workers)',
              },
              {
                step: '5',
                label: 'MP4 mux',
                desc: 'mediabunny muxes video and audio tracks into a final MP4 Blob',
              },
            ].map((row, i) => (
              <div
                key={row.step}
          ...
          <p className="text-sm leading-relaxed text-on-surface-variant">
            If you need lower-level access, use{' '}
            <code className="rounded bg-surface-container px-1.5 py-0.5 text-xs font-mono">
              lazyExportVideo()
            </code>{' '}
            — same pipeline but returns an{' '}
            <code className="rounded bg-surface-container px-1.5 py-0.5 text-xs font-mono">
              AsyncIterator
            </code>{' '}
            of frame chunks for streaming mux:
          </p>
          <CodeBlock
            language="typescript"
        ...
        <section className="mb-10">
          ...
          <div className="mb-4 rounded-md border border-outline-variant bg-amber-50 p-4">
            ...
            <p className="text-xs leading-relaxed text-amber-800">
              Web Audio API is not available in Web Workers. Audio is decoded and mixed on the{' '}
              <strong>main thread</strong> during export. For long projects this is acceptable; for
              very large projects consider chunking.
            </p>
          </div>
          <p className="mb-4 text-sm leading-relaxed text-on-surface-variant">
            ...
            and schedules Web Audio nodes beside the renderer on the same{' '}
            <code className="rounded bg-surface-container px-1.5 py-0.5 text-xs font-mono">
              PlaybackEngine
            </code>{' '}
            clock:
